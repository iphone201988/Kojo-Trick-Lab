package com.tech.kojo.ui.dashboard.tracker.session_planner.add_review_post_session
...
@HiltViewModel
class AddReviewPostSessionVm @Inject constructor(private val apiHelper: ApiHelper) : BaseViewModel() {
    val observeCommon = SingleRequestEvent<JsonObject>()

    // update session planner api
    fun updateSessionPlannerApi(url: String, request:HashMap<String, Any>) {
        viewModelScope.launch(Dispatchers.IO) {
            observeCommon.postValue(Resource.loading(null))
            runCatching {
                val response = apiHelper.apiPostForRawBody(url,request)
                if (response.isSuccessful) {
                    observeCommon.postValue(Resource.success("updateSessionPlannerApi", response.body()))
                } else {
