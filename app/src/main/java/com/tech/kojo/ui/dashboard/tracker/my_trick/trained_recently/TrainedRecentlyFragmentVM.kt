package com.tech.kojo.ui.dashboard.tracker.my_trick.trained_recently
...
@HiltViewModel
class TrainedRecentlyFragmentVM @Inject constructor(private val apiHelper: ApiHelper): BaseViewModel(){
    val observeCommon = SingleRequestEvent<JsonObject>()
    // get user progress api
    fun getUserProgressApi(data: HashMap<String, Any>,url: String) {
        viewModelScope.launch(Dispatchers.IO) {
            observeCommon.postValue(Resource.loading(null))
            runCatching {
                val response = apiHelper.apiGetWithQuery(data,url)
                if (response.isSuccessful) {
                    observeCommon.postValue(Resource.success("getUserProgressApi", response.body()))
                } else {
